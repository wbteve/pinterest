 ...
 * 搭配分类标签管理
 +------------------------------------------------------------------------------
 */
class StyleCategoryTagsAction extends CommonAction
{
	public function index()
	{
		if(isset($_REQUEST['cate_id']))
			...
			$cate_id = intval($_SESSION['style_category_tags_cate_id']);
		
		$_SESSION['style_category_tags_cate_id'] = $cate_id;
		...
		$sql = 'SELECT COUNT(DISTINCT gt.tag_id) AS tcount FROM '.C("DB_PREFIX").'style_category_tags as gct 
		...
		$sql = 'SELECT gct.*,gt.tag_name FROM '.C("DB_PREFIX").'style_category_tags as gct 
				LEFT JOIN '.C("DB_PREFIX").'goods_tags as gt ON gt.tag_id = gct.tag_id 
				WHERE '.$where;
	...
	public function insert()
	{
		...
		{
			$tag_list[$cattag['tag_name']] = array('sort'=>$cattag['sort'],'tag_id'=>$cattag['tag_id']);
		}
		...
		if(!empty($category_tags))
		{
			...
			foreach($category_tags as $category_tag)
			{
				...
				if(!empty($tag_name))
				{
					...
					{
						$tag_list[$tag_name]['sort'] = $tag_weight;
					}
					else
						$tag_list[$tag_name] = array('sort'=>$tag_weight,'tag_id'=>0);
				}
			}
		}
		...
		foreach($tag_list as $tag_name => $tag_item)
		{
			...
			{
				M()->query('UPDATE '.C("DB_PREFIX").'style_category_tags SET sort = '.$tag_item['sort'].' WHERE cate_id = '.$cate_id.' AND tag_id = '.$tag_item['tag_id']);
			}
			else
			{
				...
				if($tag_id > 0)
				{
					...
					D("StyleCategoryTags")->add($data);
				}
			}
		}
		
		$this->assign ( 'jumpUrl', U('StyleCategoryTags/index'));
