...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
		...
		if(preg_match('/[\x80-\xff]./', $uploadedFile['name']))
		{
			$sExtension = CKFinder_Connector_Utils_FileSystem::getExtension($uploadedFile['name']);
			$uploadedFile['name'] = date('YmdHis').'.'.$sExtension;
		}
		...
		if(dirname($sServerDir).'/' == $GLOBALS['baseDir'])
		{
			$now=time();
			$ymDir=$sServerDir.'/'.date('Ym',$now).'/';
			if (!file_exists($ymDir) && !is_dir($ymDir))
			{
				mkdir($ymDir,0777);
			}
			
			$dateDir=$ymDir.date('d',$now);
			...
			$sServerDir=$dateDir;
			$oRegistry->set("FileUpload_url", $this->_currentFolder->getUrl().date('Ym',$now).'/'.date('d',$now).'/');
		}
