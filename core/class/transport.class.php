 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: cls_transport.php 16881 2009-12-14 09:19:16Z liubo $
 */
...
class transport
{
    ...
    {
        $fsock_exists = function_exists('fsockopen')||function_exists('pfsockopen');
    ...
    function use_socket($url, $params, $method, $my_header)
    {
    	...
    	if(function_exists('fsockopen')) {
		    ...
		    if ($this->connect_timeout > -1)
	        {
	            $fp = @pfsockopen($url_parts['host'], $url_parts['port'], $error, $errstr, $this->connect_timeout);
	        }
	        else
	        {
	            $fp = @pfsockopen($url_parts['host'], $url_parts['port'], $error, $errstr);
	        }			
		} else {
			$fp = false;
		}                
