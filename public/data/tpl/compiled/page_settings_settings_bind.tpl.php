...
$css_list[0]['url'] = './tpl/css/setting.css';
 include template('inc/header'); ?><script type="text/javascript">
...
<ul class="bind_logo"><? if(is_array($login_modules)) { foreach($login_modules as $key => $module) { ?><li>
...
<input type="checkbox" name="syn_topic" <? if($bind_list[$key]['sync']['topic'] == 1) { ?>checked="checked"<? } ?>>发主题或提问
<input type="checkbox" name="syn_weibo" <? if($bind_list[$key]['sync']['weibo'] == 1) { ?>checked="checked"<? } ?>>发微博或商品
</form>
...
jQuery(function($){
var query = new Object();
query.is_bind = confirm("<?=$bind_exists?>");
$.ajax({
