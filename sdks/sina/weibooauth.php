 ...
 * @ignore 
 */ 
class SinaOAuthException extends Exception { 
    // pass 
} 
...
class SinaOAuthConsumer { 
    ...
    function __toString() { 
        return "SinaOAuthConsumer[key=$this->key,secret=$this->secret]"; 
    } 
} 
...
class SinaOAuthToken { 
    ...
    function to_string() { 
        return "oauth_token=" . 
            SinaOAuthUtil::urlencode_rfc3986($this->key) . 
            "&oauth_token_secret=" . 
            SinaOAuthUtil::urlencode_rfc3986($this->secret); 
    } 
 ...
 * @ignore 
 */ 
class SinaOAuthSignatureMethod { 
 ...
 * @ignore 
 */ 
class SinaOAuthSignatureMethod_HMAC_SHA1 extends SinaOAuthSignatureMethod { 
    function get_name() { 
        return "HMAC-SHA1"; 
    } 
    ...
    public function build_signature($request, $consumer, $token) { 
        ...
        //print_r( $key_parts );
		$key_parts = SinaOAuthUtil::urlencode_rfc3986($key_parts); 
 ...
 * @ignore 
 */ 
class SinaOAuthSignatureMethod_PLAINTEXT extends SinaOAuthSignatureMethod { 
    ...
    public function build_signature($request, $consumer, $token) { 
        $sig = array( 
            SinaOAuthUtil::urlencode_rfc3986($consumer->secret) 
        ); 

        if ($token) { 
            array_push($sig, SinaOAuthUtil::urlencode_rfc3986($token->secret)); 
        } else { 
            array_push($sig, ''); 
        } 
        ...
        return SinaOAuthUtil::urlencode_rfc3986($raw); 
    } 
} 
 ...
 * @ignore 
 */ 
class SinaOAuthSignatureMethod_RSA_SHA1 extends SinaOAuthSignatureMethod { 
    public function get_name() { 
        return "RSA-SHA1"; 
    } 
...
class SinaOAuthRequest { 
    ...
    public static function from_request($http_method=NULL, $http_url=NULL, $parameters=NULL) { 
        ...
        if (!$parameters) { 
            ...
            $request_headers = SinaOAuthUtil::get_headers(); 

            // Parse the query-string to find GET parameters 
            $parameters = SinaOAuthUtil::parse_parameters($_SERVER['QUERY_STRING']); 
            ...
            if ($http_method == "POST" 
                ...
                $post_data = SinaOAuthUtil::parse_parameters( 
                    getUrlContent(self::$POST_INPUT) 
                ); 
            ...
            if (@substr($request_headers['Authorization'], 0, 6) == "OAuth ") { 
                $header_parameters = SinaOAuthUtil::split_header( 
        ...
        return new SinaOAuthRequest($http_method, $http_url, $parameters); 
    } 
    ...
    public static function from_consumer_and_token($consumer, $token, $http_method, $http_url, $parameters=NULL) { 
        @$parameters or $parameters = array(); 
        $defaults = array("oauth_version" => SinaOAuthRequest::$version, 
            "oauth_nonce" => SinaOAuthRequest::generate_nonce(), 
            "oauth_timestamp" => SinaOAuthRequest::generate_timestamp(), 
    ...
    public function get_signable_parameters() { 
        ...
        return SinaOAuthUtil::build_http_query($params); 
    } 
    ...
    public function get_signature_base_string() { 
        ...
        $parts = SinaOAuthUtil::urlencode_rfc3986($parts); 
    ...
    //echo "multi=" . $multi . '`';
    if( $multi )
    	return SinaOAuthUtil::build_http_query_multi($this->parameters); 
    else 
        return SinaOAuthUtil::build_http_query($this->parameters); 
    } 
    ...
    public function to_header() { 
        ...
        foreach ($this->parameters as $k => $v) { 
            ...
            if (is_array($v)) { 
                throw new SinaOAuthException('Arrays not supported in headers'); 
            } 
            $out .= ',' . 
                SinaOAuthUtil::urlencode_rfc3986($k) . 
                '="' . 
                SinaOAuthUtil::urlencode_rfc3986($v) . 
                '"'; 
        } 
        return $out; 
    } 
...
class SinaOAuthServer { 
    ...
    private function get_version(&$request) { 
        ...
        if ($version && $version != $this->version) { 
            throw new SinaOAuthException("OAuth version '$version' not supported"); 
        } 
        return $version; 
    } 
    ...
    private function get_consumer(&$request) { 
        ...
        if (!$consumer_key) { 
            throw new SinaOAuthException("Invalid consumer key"); 
        } 

        $consumer = $this->data_store->lookup_consumer($consumer_key); 
        if (!$consumer) { 
            throw new SinaOAuthException("Invalid consumer"); 
        } 
    ...
    private function get_token(&$request, $consumer, $token_type="access") { 
        ...
        if (!$token) { 
            throw new SinaOAuthException("Invalid $token_type token: $token_field"); 
        } 
        return $token; 
    } 
    ...
    private function check_signature(&$request, $consumer, $token) { 
        ...
        if (!$valid_sig) { 
            throw new SinaOAuthException("Invalid signature"); 
        } 
    } 
    ...
    private function check_nonce($consumer, $token, $nonce, $timestamp) { 
        ...
        if ($found) { 
            throw new SinaOAuthException("Nonce already used: $nonce"); 
        } 
    } 
...
class SinaOAuthUtil { 
    ...
    public static function urlencode_rfc3986($input) { 
        if (is_array($input)) { 
            return array_map(array('SinaOAuthUtil', 'urlencode_rfc3986'), $input); 
    ...
    public static function split_header($header, $only_allow_oauth_parameters = true) { 
        ...
        while (preg_match($pattern, $header, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) { 
            ...
            if (preg_match('/^oauth_/', $header_name) || !$only_allow_oauth_parameters) { 
                $params[$header_name] = SinaOAuthUtil::urldecode_rfc3986($header_content); 
            } 
    ...
    public static function parse_parameters( $input ) { 
        ...
        foreach ($pairs as $pair) { 
            ...
            $parameter = SinaOAuthUtil::urldecode_rfc3986($split[0]); 
            $value = isset($split[1]) ? SinaOAuthUtil::urldecode_rfc3986($split[1]) : ''; 
...
class WeiboOAuth { 
    ...
    function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) { 
        $this->sha1_method = new SinaOAuthSignatureMethod_HMAC_SHA1(); 
        $this->consumer = new SinaOAuthConsumer($consumer_key, $consumer_secret); 
        if (!empty($oauth_token) && !empty($oauth_token_secret)) { 
            $this->token = new SinaOAuthConsumer($oauth_token, $oauth_token_secret); 
        } else { 
            $this->token = NULL; 
        } 
    } 
    ...
    function getRequestToken($oauth_callback = NULL) { 
        ...
        $token = SinaOAuthUtil::parse_parameters($request); 
        $this->token = new SinaOAuthConsumer($token['oauth_token'], $token['oauth_token_secret']); 
        return $token; 
    } 
    ...
    function oAuthRequest($url, $method, $parameters , $multi = false) { 
        ...
        // echo $url ; 
        $request = SinaOAuthRequest::from_consumer_and_token($this->consumer, $this->token, $method, $url, $parameters); 
...
*/
        ...
        $header_array2=array(); 
        if( $multi ) 
        	$header_array2 = array("Content-Type: multipart/form-data; boundary=" . SinaOAuthUtil::$boundary , "Expect: ");
